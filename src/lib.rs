...
pub fn run() -> Result<()> {
    ...
    if is_control(&args) {
        let mut control_args = Vec::from(&args[1..args.len()]);
        let is_global = control_args.remove_item(&"--global").is_some();
        let git_together = GitTogether::new(is_global)?;

        match *control_args.as_slice() {
            ["--version"] => {
                println!("{}", version());
            }
            ["with"] => {
        ...
        let status = process::Command::new("git").args(args).status().chain_err(
            || "failed to execute process",
        )?;

        if !status.success() {
            process::exit(status.code().unwrap_or_else(|| 1))
        }
    }
...
fn is_control(args: &[&str]) -> bool {
    args.get(0)
        .map(|x| ["with", "together"].contains(x))
        .unwrap_or_else(|| false)
}

fn version() -> String {
    let name = option_env!("CARGO_PKG_NAME").unwrap_or("git-together");
    let version = option_env!("CARGO_PKG_VERSION").unwrap_or("unknown version");
    format!("{} {}", name, version)
}

struct GitTogether<R, W> {
    reader: R,
    writer: W,
}

impl GitTogether<Config, Config> {
    fn new(global: bool) -> Result<Self> {
        let mut reader = Config::new()?;
        let writer = if global {
            reader.global()?
        } else {
            Config::new()?
        };
        Ok(GitTogether { reader, writer })
    }
}

impl<R: Reader, W: Writer> GitTogether<R, W> {
    fn active(&self) -> Result<Vec<String>> {
        self.reader.get(&namespaced("active")).map(|active| {
            active.split('+').map(|s| s.into()).collect()
        })
    }

    fn set_active(&mut self, inits: &[&str]) -> Result<Vec<Author>> {
        let authors = inits.iter().map(|inits| self.author(inits)).collect::<Result<Vec<_>>>()?;
        self.writer.set(&namespaced("active"), &inits.join("+"))?;

        self.backup_original_user()?;
        if let Some(author) = authors.get(0) {
            self.writer.set("user.name", &author.name)?;
            self.writer.set("user.email", &author.email)?;
        }

        Ok(Vec::new())
    }

    fn backup_original_user(&mut self) -> Result<()> {
        if let Ok(name) = self.reader.get("user.name") {
            let key = namespaced("user.name");
            self.reader
                .get(&key)
                .map(|_| ())
                .or_else(|_| self.writer.set(&key, &name))?;
        }

        if let Ok(email) = self.reader.get("user.email") {
            let key = namespaced("user.email");
            self.reader
                .get(&key)
                .map(|_| ())
                .or_else(|_| self.writer.set(&key, &email))?;
        }

        Ok(())
    }

    fn author(&self, inits: &str) -> Result<Author> {
        let domain = self.reader.get(&namespaced("domain")).ok();
        self.reader
            .get(&namespaced(&format!("authors.{}", inits)))
            .chain_err(|| format!("author not found for '{}'", inits))
            .and_then(|raw| Author::new(&raw, domain.as_ref().map(String::as_str)))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_getting_active() {
        let mut reader = FakeConfig::default();
        reader.set(&namespaced("active"), "foo+bar").unwrap();
        let writer = FakeConfig::default();
        let gt = GitTogether { reader, writer };

        assert_eq!(gt.active().unwrap(), vec!["foo", "bar"]);
    }

    #[test]
    fn test_setting_active() {
        let mut reader = FakeConfig::default();
        reader.set("user.name", "Bobbie Draper").unwrap();
        reader.set("user.email", "bdraper@mars.mil").unwrap();
        let writer = FakeConfig::default();
        let mut gt = GitTogether { reader, writer };

        gt.set_active(&["jh", "nn"]).unwrap();

        assert_eq!(gt.active().unwrap(), vec!["jh", "nn"]);
        assert_eq!(gt.writer["user.name"], "James Holden");
        assert_eq!(gt.writer["user.email"], "jholden@rocinante.com");
        assert_eq!(gt.writer["git-together.user.name"], "Bobbie Draper");
        assert_eq!(gt.writer["git-together.user.email"], "bdraper@mars.mil");
    }

    #[test]
    fn test_getting_authors() {
        let reader = FakeConfig::default();
        let writer = FakeConfig::default();
        let gt = GitTogether { reader, writer };

        assert!(gt.author("x").is_err());
    ...
    impl FakeConfig {
        fn new(data: &[(&str, &str)]) -> Self {
            let data = data.iter().map(|&(k, v)| (k.into(), v.into())).collect();
            FakeConfig { data }
        }
    }
    ...
    impl Default for FakeConfig {
        fn default() -> Self {
            Self::new(
                &[
                    ("git-together.domain", "rocinante.com"),
    ...
    impl git::Reader for FakeConfig {
        fn get(&self, name: &str) -> Result<String> {
            self.data.get(name.into()).cloned().ok_or_else(|| "".into())
        }
    ...
    impl git::Writer for FakeConfig {
        fn add(&mut self, _name: &str, _value: &str) -> Result<()> {
            unimplemented!();
        }
    ...
    impl<'a> Index<&'a str> for FakeConfig {
        type Output = String;
